...
# %%
def cv_density(models, std_tensors, mean_tensors, rnge, num_steps, figname, xlabel, ylabel, save, save_path, colorscheme, markerscheme):
    ...
    for model in models:
        ...
        for data, value in std_tensors[model].items():
            seg_std = std_tensors[model][data]
            seg_mean = mean_tensors[model][data]
            mask = TF.resize(transforms.ToTensor()(~masks[data]), (seg_std.shape[-2:]))[0].numpy()
            temp = np.ma.array(seg_std, mask = mask)
            temp2 = np.ma.array(seg_mean, mask = mask)
            std_data.append(temp[~temp.mask].data)
            mean_data.append(temp2[~temp2.mask].data)
        ...
        kde = KernelDensity(kernel = 'gaussian', bandwidth = bandwidth).fit(cv_data)
...
rnge = (0, 5)
num_steps = 1000
figtitle = "All Model DB CV"
save_path = 'results/Images/All_Models'
cv_density(models, std_tensors = std_db_tensors, mean_tensors = mean_db_tensors, rnge = rnge, num_steps = num_steps, figname = figtitle,xlabel = 'CV', ylabel = 'Density',  save = True, save_path = save_path, colorscheme = colorscheme, markerscheme = markerscheme)

figtitle = "Base Model DB CV"
save_path = 'results/Images/All_Models'
cv_density(base_models, std_tensors = std_db_tensors, mean_tensors = mean_db_tensors, rnge = rnge, num_steps = num_steps, figname = figtitle, xlabel = 'CV', ylabel = 'Density', save = True, save_path = save_path, colorscheme = colorscheme, markerscheme = markerscheme)

figtitle = "LF HFT Model DB CV"
save_path = 'results/Images/All_Models'
cv_density(LF_HFT_models, std_tensors = std_db_tensors, mean_tensors = mean_db_tensors, rnge = rnge, num_steps = num_steps, figname = figtitle, xlabel = 'CV', ylabel = 'Density', save = True, save_path = save_path, colorscheme = colorscheme, markerscheme = markerscheme)
figtitle = "LF LFT Model DB CV"
save_path = 'results/Images/All_Models'
cv_density(LF_LFT_models, std_tensors = std_db_tensors, mean_tensors = mean_db_tensors, rnge = rnge, num_steps = num_steps, figname = figtitle, xlabel = 'CV', ylabel = 'Density', save = True, save_path = save_path, colorscheme = colorscheme, markerscheme = markerscheme)
figtitle = "MF Model DB CV"
save_path = 'results/Images/All_Models'
cv_density(mf_models, std_tensors = std_db_tensors, mean_tensors = mean_db_tensors, rnge = rnge, num_steps = num_steps, figname = figtitle, xlabel = 'CV', ylabel = 'Density', save = True, save_path = save_path, colorscheme = colorscheme, markerscheme = markerscheme)

# %%
rnge = (0, 2)
num_steps = 1000
figtitle = "All Model ROT CV"
save_path = 'results/Images/All_Models'
cv_density(models, std_tensors = std_rot_tensors, mean_tensors = mean_rot_tensors, rnge = rnge, num_steps = num_steps, figname = figtitle, xlabel = 'CV', ylabel = 'Density', save = True, save_path = save_path, colorscheme = colorscheme, markerscheme = markerscheme)

figtitle = "Base Model ROT CV"
save_path = 'results/Images/All_Models'
cv_density(base_models, std_tensors = std_rot_tensors, mean_tensors = mean_rot_tensors, rnge = rnge, num_steps = num_steps, figname = figtitle, xlabel = 'CV', ylabel = 'Density', save = True, save_path = save_path, colorscheme = colorscheme, markerscheme = markerscheme)

figtitle = "LF HFT Model ROT CV"
save_path = 'results/Images/All_Models'
cv_density(LF_HFT_models, std_tensors = std_rot_tensors, mean_tensors = mean_rot_tensors, rnge = rnge, num_steps = num_steps, figname = figtitle, xlabel = 'CV', ylabel = 'Density', save = True, save_path = save_path, colorscheme = colorscheme, markerscheme = markerscheme)
figtitle = "LF LFT Model ROT CV"
save_path = 'results/Images/All_Models'
cv_density(LF_LFT_models, std_tensors = std_rot_tensors, mean_tensors = mean_rot_tensors, rnge = rnge, num_steps = num_steps, figname = figtitle, xlabel = 'CV', ylabel = 'Density', save = True, save_path = save_path, colorscheme = colorscheme, markerscheme = markerscheme)
figtitle = "MF Model ROT CV"
save_path = 'results/Images/All_Models'
cv_density(mf_models, std_tensors = std_rot_tensors, mean_tensors = mean_rot_tensors, rnge = rnge, num_steps = num_steps, figname = figtitle, xlabel = 'CV', ylabel = 'Density', save = True, save_path = save_path, colorscheme = colorscheme, markerscheme = markerscheme)

# %%
   # Single Model     
def cv_single_density(model, std_tensors, mean_tensors, rnge, num_steps, figname, xlabel, ylabel, save, save_path, colorscheme):
    bandwidth = (rnge[1] - rnge[0])/num_steps # bandwidth to approx density, must be small b/c our data lies in a small range
    fig, ax = plt.subplots(1, 1, figsize = (15, 10))
    ...
    for data, value in std_tensors[model].items():
        ...
        temp = np.ma.array(seg_std, mask = mask)
        temp2 = np.ma.array(seg_mean, mask = mask)
        std_data = temp[~temp.mask].data
        mean_data = temp2[~temp2.mask].data
        cv_data = (std_data/mean_data)
        ...
        X_plot = np.linspace(rnge[0], rnge[1], num_steps)[:, np.newaxis]
        log_dens = kde.score_samples(X_plot)
        ax.plot(X_plot[:, 0], np.exp(log_dens),c = colorscheme[data], label = data , alpha = .6, linewidth = 1.5)
...
for model in models:
    figtitle = f"{model} DB CV"
    rnge = (0, 5)
    save_path = 'results/Images/Single_Models'
    cv_single_density(model, std_tensors = std_db_tensors, mean_tensors = mean_db_tensors, rnge = rnge, num_steps = num_steps, figname = figtitle, xlabel = 'CV', ylabel = 'Density', save = True, save_path = save_path, colorscheme = colorscheme_im)
    
    figtitle = f"{model} ROT CV"
    rnge = (0, 2)
    save_path = 'results/Images/Single_Models'
    cv_single_density(model, std_tensors = std_rot_tensors, mean_tensors = mean_rot_tensors, rnge = rnge, num_steps = num_steps, figname = figtitle, xlabel = 'CV', ylabel = 'Density', save = True, save_path = save_path, colorscheme = colorscheme_im,)
