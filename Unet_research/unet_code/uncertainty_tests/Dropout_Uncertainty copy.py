...
from utils.utils_unet import Dropblock2d_ichan, UNet
    ...
    with torch.no_grad():
        for batch_idx, (image_batch, gt, mask) in enumerate(dataloader):
            ...
            if save:
                ...
                #don't save all tensors for this run !~!!!~~
                torch.save(tensors[0:25].clone(), os.path.join(im_path, 'tensors.pt'))
...
if __name__=='__main__':
    ...
    unet = UNet(**unet_params, dropblock_ichan = True)
    ...
    uncertainty_root = results_root + '/Dropout_MonteCarlo_dchan'
