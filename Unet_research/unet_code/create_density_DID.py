...
# %%
def dvud_cv(model, std_tensors, mean_tensors, target_tensors, mask_tensors, rnge, num_steps, figname, xlabel, ylabel, save, save_path):
    ...
    aggdata = []
    aggdata2 = []
    for im_num, value in std_tensors[model].items():
        seg = std_tensors[model][im_num][0][0]
        seg_mean = mean_tensors[model][im_num]
        target = TF.resize(transforms.ToTensor()(~target_tensors[im_num]), (seg.shape[-2:]))[0].numpy()
        mask = TF.resize(transforms.ToTensor()(mask_tensors[im_num]), (seg.shape[-2:]))[0].numpy()
        dilated = cv2.erode(target, np.ones((3,3),np.uint8))
        inv_dilated = 1 - (mask * dilated)
        temp = np.ma.array(seg,mask = inv_dilated)
        temp2 = np.ma.array(seg_mean, mask = inv_dilated)
    ...
    aggdata = aggdata.flatten()
    aggdata = aggdata[~np.isnan(aggdata)]
    aggdata = aggdata[:, np.newaxis]

    kde = KernelDensity(kernel = 'gaussian', bandwidth = bandwidth).fit(aggdata)
    X_plot = np.linspace(rnge[0], rnge[1], num_steps)[:, np.newaxis]
    log_dens = kde.score_samples(X_plot)
    ax.plot(X_plot[:, 0], np.exp(log_dens), 'b', label = 'Inverse Dilated' , alpha = .9, linewidth = 1.5)
    
    # dilated
    aggdata = []
    aggdata2 = []
    for im_num, value in std_tensors[model].items():
        seg = std_tensors[model][im_num]
        seg_mean = mean_tensors[model][im_num]
        target = TF.resize(transforms.ToTensor()(~target_tensors[im_num]), (seg.shape[-2:]))[0].numpy()
        dilated = cv2.erode(target, np.ones((3,3),np.uint8))
        temp = np.ma.array(seg,mask = dilated)
    ...
    X_plot = np.linspace(rnge[0], rnge[1], num_steps)[:, np.newaxis]
    log_dens = kde.score_samples(X_plot)
    ax.plot(X_plot[:, 0], np.exp(log_dens), 'r', label = 'Dilated' , alpha = .9, linewidth = 1.5)
...
# %%
def dvud_std(model, std_data, target_data, mask_data, rnge, num_steps, figname, xlabel, ylabel, save, save_path):
    # overlapping std DB density
    bandwidth = (rnge[1] - rnge[0])/num_steps # bandwidth to approx density, must be small b/c our data lies in a small range
    fig, ax = plt.subplots(1, 1, figsize = (15, 10))
    ...
    ddata = []
    idata = []
    for im_num, value in std_data[model].items():
        seg = std_data[model][im_num]
        target = TF.resize(transforms.ToTensor()(~target_data[im_num]), (seg.shape[-2:]))[0].numpy()
        dilated = cv2.erode(target, np.ones((3,3),np.uint8))
        temp = np.ma.array(seg,mask = dilated)
        ddata.append(temp[~temp.mask].data)
        
        mask = TF.resize(transforms.ToTensor()(mask_data[im_num]), (seg.shape[-2:]))[0].numpy()
        
        inv_dilated = 1 - (mask * dilated)
        temp = np.ma.array(seg ,mask = inv_dilated)
        idata.append(temp[~temp.mask].data)
    ddata = np.concatenate(ddata).flatten()[:, np.newaxis]
    idata = np.concatenate(idata).flatten()[:, np.newaxis]
    kded = KernelDensity(kernel = 'gaussian', bandwidth = bandwidth).fit(ddata)
    kdei = KernelDensity(kernel = 'gaussian', bandwidth = bandwidth).fit(idata)
    X_plot = np.linspace(rnge[0], rnge[1], num_steps)[:, np.newaxis]
    log_dens_d = kded.score_samples(X_plot)
    log_dens_i = kdei.score_samples(X_plot)
    #save_info[model] = log_dens
    ax.plot(X_plot[:, 0], np.exp(log_dens_d), 'r', label = 'Dilated' , alpha = .9, linewidth = 1.5)
    ax.plot(X_plot[:, 0], np.exp(log_dens_i), 'b', label = 'Inverse Dilated' , alpha = .9, linewidth = 1.5)
    ax.set_ylim(0, max(np.exp(log_dens_d))*1.5)
...
for model in models:
    print(model)
    figtitle = f"{model} Dilated Vs Inverse Dilated DB STD"
    rnge = (0, .3)
    save_path = 'results/Images/DID'
    dvud_std(model, std_data = std_db_tensors, target_data = targets, mask_data = masks, rnge = rnge, num_steps = num_steps, figname = figtitle, xlabel = 'STD', ylabel= 'Density', save = True, save_path = save_path)
    
    figtitle = f"{model} Dilated Vs Inverse Dilated ROT STD"
    rnge = (0, .2)
    save_path = 'results/Images/DID'
    dvud_std(model, std_data = std_rot_tensors, target_data = targets, mask_data = masks, rnge = rnge, num_steps = num_steps, figname = figtitle, xlabel = 'STD', ylabel = 'Density', save = True, save_path = save_path)
...
for model in models:
    print(model)
    figtitle = f"{model} Dilated Vs Inverse Dilated DB CV"
    rnge = (0, 4)
    save_path = 'results/Images/DID'
    dvud_cv(model, std_tensors = std_db_tensors, mean_tensors=mean_db_tensors, target_tensors = targets, mask_tensors = masks, rnge = rnge, num_steps = num_steps, figname = figtitle, xlabel = 'CV', ylabel= 'Density', save = True, save_path = save_path)
    
    figtitle = f"{model} Dilated Vs Inverse Dilated ROT CV"
    rnge = (0, 3)
    save_path = 'results/Images/DID'
    dvud_cv(model, std_tensors = std_rot_tensors, mean_tensors = mean_rot_tensors, target_tensors = targets, mask_tensors = masks, rnge = rnge, num_steps = num_steps, figname = figtitle, xlabel = 'CV', ylabel = 'Density', save = True, save_path = save_path)
